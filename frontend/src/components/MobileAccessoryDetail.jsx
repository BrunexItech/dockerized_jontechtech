// src/pages/MobileAccessoryDetail.jsx
...
export default function MobileAccessoryDetail() {
  const { id } = useParams(); // /accessories/:id -> MobileAccessory PK
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setErr("");
        const item = await api.accessories.get(id);
        if (!cancelled) setData(item);
      } catch (e) {
        if (!cancelled) {
          setErr(e?.message || "Failed to load accessory.");
          toast.error(e?.message || "Failed to load accessory");
        }
      } finally {
