...
export default function TabletDetail() {
  const { id } = useParams(); // /tablet/:id  -> this is the Tablet PK
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setErr("");
        const tablet = await api.tablets.get(id);
        if (!cancelled) setData(tablet);
      } catch (e) {
        if (!cancelled) {
          setErr(e?.message || "Failed to load tablet.");
          toast.error(e?.message || "Failed to load tablet");
        }
      } finally {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      setAdding(true);
      // add to cart using the linked Product PK (NOT the Tablet PK)
      const updatedCart = await api.cart.add(data.product_id, 1);

      // Broadcast updated count to header badge (same pattern as ProductDetail)
  ...
  const {
    ...
    specs_text,
    slug,
    product_id, // used for cart/checkout
  } = data;
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          <p className="text-blue-600 font-semibold text-lg mb-6">
            {price_display ||
              (price_max_ksh ? `${price_min_ksh} – ${price_max_ksh} KSh` : `${price_min_ksh} KSh`)}
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            <DetailItem label="RAM" value={ram_gb ? `${ram_gb} GB` : "—"} />
            <DetailItem label="Storage" value={storage_gb ? `${storage_gb} GB` : "—"} />
            <DetailItem label="Display" value={display_inches ? `${display_inches}"` : "—"} />
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              onClick={() => {
                ...
                // Pass product_id so checkout/cart can identify the purchasable Product
