...
export default function NewIphoneDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setErr("");
        const device = await api.newIphones.get(id);
        if (!cancelled) setData(device);
      } catch (e) {
        if (!cancelled) {
          setErr(e?.message || "Failed to load iPhone.");
          toast.error(e?.message || "Failed to load iPhone");
        }
      } finally {
  ...
  const {
    name, price_display, new_price_ksh, old_price_ksh,
    specs_text, image, banner_image, product_id,
  } = data;
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div className="w-full h-96 bg-white border rounded-2xl flex items-center justify-center">
          <img
            src={image || banner_image || FallbackImg}
            alt={name}
        ...
        <div>
          ...
          <p className="text-blue-600 font-semibold text-lg mb-6">
            {price_display || (old_price_ksh ? `${new_price_ksh} – ${old_price_ksh} KSh` : `${new_price_ksh} KSh`)}
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            <DetailItem label="Specs" value={specs_text || "—"} />
            <DetailItem label="Slug" value={data.slug || "—"} />
            <DetailItem label="Banner" value={banner_image ? "Yes" : "—"} />
            <DetailItem label="Available" value={product_id ? "Yes" : "No"} />
          </div>

          <div className="flex gap-3">
            <button
              className={`px-5 py-2 rounded-xl ${product_id ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-gray-200 text-gray-500 cursor-not-allowed"}`}
            ...
            <button
              className={`px-5 py-2 rounded-xl ${product_id ? "bg-gray-100 hover:bg-gray-200 text-gray-900" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
              onClick={() => {
                if (!product_id) {
                  toast.error("This item is not available for purchase yet.");
                  return;
