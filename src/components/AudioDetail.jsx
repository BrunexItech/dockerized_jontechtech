...
export default function AudioDetail() {
  const { id } = useParams(); // /audio/:id -> AudioDevice PK
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setErr("");
        const device = await api.audio.get(id);
        if (!cancelled) setData(device);
      } catch (e) {
        if (!cancelled) {
          setErr(e?.message || "Failed to load audio device.");
          toast.error(e?.message || "Failed to load audio device");
        }
      } finally {
  ...
  const {
    ...
    specs_text, wireless, anc, battery_life_hours, slug, product_id,
  } = data;
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            <DetailItem label="Wireless" value={wireless ? "Yes" : "No"} />
            <DetailItem label="ANC" value={anc ? "Yes" : "No"} />
            <DetailItem label="Battery Life" value={battery_life_hours ? `${battery_life_hours} hrs` : "—"} />
