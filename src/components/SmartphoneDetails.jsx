// src/pages/SmartphoneDetail.jsx
...
export default function SmartphoneDetail() {
  const { id } = useParams(); // /smartphone/:id -> Smartphone PK
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setErr("");
        const phone = await api.smartphones.get(id);
        if (!cancelled) setData(phone);
      } catch (e) {
        if (!cancelled) {
          setErr(e?.message || "Failed to load smartphone.");
          toast.error(e?.message || "Failed to load smartphone");
        }
      } finally {
  ...
  const {
    name, brand_display, image, price_display, price_min_ksh, price_max_ksh,
    ram_gb, storage_gb, camera_mp, battery_mah, display_inches, display_type,
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            ...
            <DetailItem label="Camera" value={camera_mp ? `${camera_mp} MP` : "—"} />
            <DetailItem label="Battery" value={battery_mah ? `${battery_mah} mAh` : "—"} />
